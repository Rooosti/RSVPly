...
favorites = db.Table('favorites', db.Column('user_id', db.Integer, db.ForeignKey('user.id')), db.Column('event_id', db.Integer, db.ForeignKey('event.id')))

class Event(db.Model): #Event created by user
    ...
    ratings = db.relationship('Rating', backref='rated_event', cascade="all, delete-orphan") #relationship with rating
    comments = db.relationship('Comment', backref='commented_event', cascade="all, delete-orphan") #relationship with comment
...
class User(db.Model, UserMixin): #Account info
    ...
    favorites = db.relationship('Event', secondary=favorites, backref='favoriters')
...
class Comment (db.Model):
    ...
    event = db.relationship('Event', backref="specific_comment")
